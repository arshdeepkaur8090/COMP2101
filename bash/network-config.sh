...
firstCommand=`ip a |awk '/: e/{gsub(/:/,"");print $2}'`
LANAdress=`ip a s $firstCommand |awk '/inet /{gsub(/\/.*/,"");print $2}'`
LANName=`getent hosts $LANAdress | awk '{print $2}'`
extAddress=`curl -s icanhazip.com`
extName=`getent hosts $extAddress | awk '{print $2}'`

cat <<EOF
Hostname        : `hostname`
LAN Address     : $LANAdress
LAN Hostname    : $LANName
External IP     : $extAddress
...
routAddress=`ip route show default | awk '{print $3; exit}'`
